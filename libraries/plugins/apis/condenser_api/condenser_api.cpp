...
namespace detail
{
   ...
   {
      CHECK_ARG_SIZE(1)
      FC_ASSERT(_sps_api, "sps_api not enabled");
      
      const sps::find_proposals_return &proposals = _sps_api->find_proposals(sps::find_proposals_args{args[0].as<flat_set<uint64_t> >()});

      find_proposals_return result;
      for (const auto &proposal : proposals) 
      {
         result.push_back(api_proposal_object(proposal));
      };
   ...
   DEFINE_API_IMPL(condenser_api_impl, list_proposals) 
   {
      CHECK_ARG_SIZE(5);
      FC_ASSERT(_sps_api, "sps_api not enabled");
      const sps::list_proposals_return &proposals = _sps_api->list_proposals(sps::list_proposals_args{args[0], args[1].as<sps::order_by_type>(), args[2].as<sps::order_direction_type>(), args[3].as<uint16_t>(), args[4].as<int8_t>()});
   ...
   DEFINE_API_IMPL(condenser_api_impl, list_voter_proposals) 
   {
      CHECK_ARG_SIZE(5);
      FC_ASSERT(_sps_api, "sps_api not enabled");
      const sps::list_voter_proposals_return &proposals = _sps_api->list_voter_proposals(sps::list_voter_proposals_args{args[0].as<account_name_type>(), args[1].as<sps::order_by_type>(), args[2].as<sps::order_direction_type>(), args[3].as<uint16_t>(), args[4].as<int8_t>()});
...
void condenser_api::api_startup()
{
   ...
   auto sps = appbase::app().find_plugin< sps::sps_api_plugin >();
   if (sps != nullptr)
   {
      my->_sps_api = sps->api;
   }
