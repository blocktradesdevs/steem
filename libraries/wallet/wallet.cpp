...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const steem::protocol::chain_id_type& _steem_chain_id, fc::api< remote_node_api > rapi, fc::api< remote_node_api > _sps_api )
      : self( s ),
        _remote_api( rapi ),
        _sps_api( _sps_api )
   {
   ...
   fc::api< remote_node_api >              _sps_api;
...
wallet_api::wallet_api(const wallet_data& initial_data, const steem::protocol::chain_id_type& _steem_chain_id, fc::api< remote_node_api > rapi, fc::api< remote_node_api > _sps_api)
...
condenser_api::legacy_signed_transaction wallet_api::follow( string follower, string following, set<string> what, bool broadcast )
{
                                                                         ...
                                                                         int _active)
   {
      ...
      try {
         return my->_sps_api->list_proposals(args);
                                                                                     ...
                                                                                     int _active)
   {
      ...
      try {
         return my->_sps_api->list_voter_proposals(args);
      } catch( fc::exception& _e) {
   ...
   steem::plugins::sps::find_proposals_return wallet_api::find_proposals(flat_set<uint64_t> _ids)
   {
      ...
      try {
         return my->_sps_api->find_proposals(args);
      } catch( fc::exception& _e) {
