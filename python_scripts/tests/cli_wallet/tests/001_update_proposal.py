...
if __name__ == "__main__":
    try:
        ...
        proposals_before = len(find_creator_proposals(creator, last_message_as_json( wallet.list_voter_proposals(creator, "creator", "asc", 20, -1))))
        ...
        resp = find_creator_proposals(creator, last_message_as_json(wallet.list_proposals(creator, "creator", "asc", 50, -1)))
        new_proposal_id = 0
        for r in resp:
            if r["id"] > new_proposal_id:
                new_proposal_id = r["id"]

        proposals_middle = len(find_creator_proposals(creator, last_message_as_json( wallet.list_voter_proposals(creator, "creator", "asc", 20, -1))))
        log.info("proposals_middle {0}".format(proposals_middle))

        wallet.update_proposal_votes(creator, [new_proposal_id], "true", "true")
        proposals_after = len(find_creator_proposals(creator, last_message_as_json( wallet.list_voter_proposals(creator, "creator", "asc", 20, -1))))
