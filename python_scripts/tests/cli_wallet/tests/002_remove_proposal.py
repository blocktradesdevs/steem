...
if __name__ == "__main__":
    try:
        ...
        proposals_before = len(find_creator_proposals(creator, last_message_as_json( wallet.list_proposals(creator, "creator", 20, "all"))))
        ...
        proposals_middle = len(find_creator_proposals(creator, last_message_as_json(wallet.list_proposals(creator, "creator", 20, "all"))))
        log.info("proposals_middle {0}".format(proposals_middle))

        wallet.remove_proposal(creator, [new_proposal_id], "true", "true")
        proposals_after = len(find_creator_proposals(creator, last_message_as_json(wallet.list_proposals(creator, "creator", 20, "all"))))
