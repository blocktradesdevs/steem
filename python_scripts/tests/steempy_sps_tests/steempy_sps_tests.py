...
def test_list_proposals(node, account, wif, subject):
    ...
    proposals = s.list_proposals(account, "by_creator", 1000, "inactive")
    found = None
    ...
    proposals = s.list_proposals(account, "by_creator", 1000, "active")
    found = None
    for proposal in proposals:
        if proposal["subject"] == subject:
            found = proposal
    ...
    proposals = s.list_proposals(account, "by_creator", 1000, "all")
...
def test_list_voter_proposals(node, account, wif, subject):
    ...
    s = Steem(nodes = [node], no_broadcast = False, keys = [wif])
    voter_proposals = s.list_voter_proposals(account, "by_creator", 1000, "inactive")
