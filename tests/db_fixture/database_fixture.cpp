                           ...
                           asset daily_pay, const fc::ecc::private_key& key )
{
   ...
   const auto& proposal_idx = this->db-> template get_index< proposal_index >().indices(). template get< by_creator >();
...
template< typename T>
void t_proposal_database_fixture< T >::remove_proposal(account_name_type _deleter, flat_set<int64_t> _proposal_id){
...
template void t_proposal_database_fixture< clean_database_fixture >::remove_proposal(account_name_type _deleter, flat_set<int64_t> _proposal_id);
...
template void t_proposal_database_fixture< database_fixture >::remove_proposal(account_name_type _deleter, flat_set<int64_t> _proposal_id);
